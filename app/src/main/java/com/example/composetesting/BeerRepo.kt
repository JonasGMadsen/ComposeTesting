...
class BeerRepo {
    ...
    fun getBeers() {
        ...
        beerService.getAllBeers().enqueue(object : Callback<List<Beer>> {
            override fun onResponse(call: retrofit2.Call<List<Beer>>, response: Response<List<Beer>>) {
            ...
            override fun onFailure(call: retrofit2.Call<List<Beer>>, t: Throwable) {
                reloadingLiveData.value = false
    ...
    fun addBeer(beer: Beer) {
        beerService.addBeer(beer).enqueue(object : Callback<Beer> {
            override fun onResponse(call: retrofit2.Call<Beer>, response: Response<Beer>) {
                if (response.isSuccessful) {
            ...
            override fun onFailure(call: retrofit2.Call<Beer>, t: Throwable) {
                errorMessageLiveData.postValue(t.message)
            }
        })
    }
